...
///
/// Mirrors the common Unix convention: a user-specific subdirectory under the
/// system temp directory so sockets from different users do not collide.
...
pub fn remove_node_socket(port: u16) {
    crate::infra::best_effort::remove_file(ratatui_socket_path(port));
}
