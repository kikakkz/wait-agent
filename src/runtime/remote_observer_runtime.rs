...
impl RemoteObserverRuntime {
    ...
    /// Feed bytes that should appear immediately on the modeled screen without
    /// updating the remote output sequence counter. Used for local echo of
    /// keyboard input in remote sessions so typed characters are visible before
    /// the remote peer responds.
    pub fn feed_local_echo(&mut self, bytes: &[u8]) {
        let replies = self.terminal.feed_and_collect_replies(bytes);
        self.pending_replies.extend_from_slice(&replies);
        self.has_visible_output = true;
    }
